...
Limits the size of data the Image Server is allowed to send to the [!DNL Platform Server]. Effectively, this limits the size of the encoded/compressed response image that Image Serving can return to the client by way of HTTP (Mbytes).
...
Limits the size of images the Image Server writes to files with the `req=saveToFile` command. Integer value larger than 0 in millions of pixels. An error is returned if the file save operation would exceed that limit. Default is 100 million pixels.

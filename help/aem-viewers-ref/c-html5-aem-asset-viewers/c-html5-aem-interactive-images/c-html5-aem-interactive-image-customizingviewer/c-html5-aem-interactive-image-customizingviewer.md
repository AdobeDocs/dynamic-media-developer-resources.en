...
The viewer supports two mechanisms of creating responsive designed CSS: CSS markers and standard CSS media queries. You can use these markers or queries independently or together.

**CSS markers**

To help create responsive designed CSS, the viewer supports CSS markers. These markers are special CSS classes that are dynamically assigned to the top-level viewer container element. They are based on the runtime viewer size and the input type used on the current device.
